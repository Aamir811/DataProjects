...
# Assuming you have already trained your model and have it stored in the variable 'trained_model'
# And assuming you have loaded and preprocessed df1 to df11 and combined them to train the model.

# Let's assume you want to test on df11 (2023 data) for example.
# Make sure df11 has been preprocessed (columns removed, conference extracted and converted to numerical,
# Average.Time.of.Possession.per.Game converted to numerical)

# Example: Assuming df11 is your new DataFrame for testing
df_new = df8.copy() # Use a copy to avoid modifying the original df11 if you need it elsewhere

# --- Prepare the new DataFrame for testing ---

# 1. Ensure it has the same columns as the training data (excluding 'Champion')
#    Drop the target column if it exists in the new dataframe
...
# 2. Ensure the columns are in the same order as the training data features (X_train)
#    You can get the column names from the training data's feature set (X)
#    Assuming 'combined_df' was used for training, you can get the training columns like this:
#    training_columns = combined_df.drop(columns=['Champion', 'Team']).columns
#    X_new = X_new[training_columns]

# 3. Apply the same preprocessing steps as done for training data
#    (e.g., time_to_seconds conversion for 'Average.Time.of.Possession.per.Game')
#    df_new['Average.Time.of.Possession.per.Game'] = df_new['Average.Time.of.Possession.per.Game'].apply(time_to_seconds)
